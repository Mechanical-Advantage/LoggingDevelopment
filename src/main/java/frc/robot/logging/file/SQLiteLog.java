...
public class SQLiteLog implements LogDataReceiver {
  ...
  private static final String filename = "/home/lvuser/robotlog.db";
      ...
      try {
        ...
        for (Map.Entry<String, Object> field : oldMap.entrySet()) {
          if (!newMap.containsKey(field.getKey())) {
            table.insert(entry.getTimestamp(), field.getKey(), LoggableType.identify(field.getValue()).ordinal(), null);
          }
        }
