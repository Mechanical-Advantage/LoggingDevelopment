...
public class SysIdCommand extends CommandBase {
  ...
  private SetDriveTrainVoltage driveTrainSetter;
  private SetMechanismVoltage mechanismSetter;
  private GetDriveTrainSysIdData driveTrainGetter;
  private GetMechanismSysIdData mechanismGetter;

  private double startTime;
  private String data;

  /** Creates a new SysIdCommand for a drive train. */
  public SysIdCommand(Subsystem subsystem, SetDriveTrainVoltage driveTrainSetter,
      GetDriveTrainSysIdData driveTrainGetter) {
  ...
  public SysIdCommand(Subsystem subsystem, SetMechanismVoltage mechanismSetter,
      GetMechanismSysIdData mechanismGetter) {
    addRequirements(subsystem);
    isDriveTrain = false;
  ...
  public void execute() {
    ...
    if (!correctTest) {
      if (isDriveTrain) {
        driveTrainSetter.set(0.0, 0.0);
      } else {
        mechanismSetter.set(0.0);
      }
      return;
    }
    ...
    if (isDriveTrain) {
      driveTrainSetter.set(primaryVoltage, secondaryVoltage);
      ...
      mechanismSetter.set(primaryVoltage);
  ...
  public void end(boolean interrupted) {
    if (data.length() > 0) {
      ...
      System.out.println("Saved " + Long.toString(Math.round(data.length() / 1024.0)) + " KB of data.");
    } else {
  ...
  public static class DriveTrainSysIdData {
     ...
     */
    public DriveTrainSysIdData(double leftPosRad, double rightPosRad, double leftVelRadPerSec, double rightVelRadPerSec,
        double gyroPosRad, double gyroVelRadPerSec) {
  ...
  @FunctionalInterface
  public static interface SetDriveTrainVoltage {
    void set(double leftVoltage, double rightVoltage);
  }

  @FunctionalInterface
  public static interface GetDriveTrainSysIdData {
    DriveTrainSysIdData get();
  }
  ...
  @FunctionalInterface
  public static interface SetMechanismVoltage {
    void set(double voltage);
  }

  @FunctionalInterface
  public static interface GetMechanismSysIdData {
    MechanismSysIdData get();
  }
}
